use crate::{db::Database, UnifiContract, CHAIN_SPEC, derive::derive_transactions};
use eyre::OptionExt;
...
use reth_primitives::{
    ...
    Address, Block, BlockWithSenders, Bytes, EthereumHardfork, Header, Receipt, TransactionSigned, B256, U256,
};
use reth_revm::{
...
pub async fn execute_block<Pool: TransactionPool>(
    ...
    header: &UnifiContract::BlockHeader,
    block_data: Bytes,
    ...
    if header.unifiChainId != U256::from(chain_id) {
    ...
    // Decode transactions
    let transactions = derive_transactions(pool, tx, block_data, block_data_hash, chain_id).await?;
...
fn construct_header(db: &Database, header: &UnifiContract::BlockHeader) -> eyre::Result<Header> {
    let parent_block = if !header.sequenceNum.is_zero() {
        db.get_block(header.sequenceNum - U256::from(1))?
    } else {
        None
    };

    let block_number = u64::try_from(header.sequenceNum)?;
...
mod tests {
    ...
    use crate::{db::Database, execute::execute_block, UnifiContract::BlockHeader, CHAIN_ID};
...
[
   {
    ...
    async fn setup_db() -> eyre::Result<()> {
        ...
        // Unifi contract deployment transaction sent using calldata
        let (_, _, results) = execute_transaction(
            ...
            Transaction::Eip2930(TxEip2930 {
                ...
                to: TxKind::Create,
                // Unifi contract bytecode
                input: bytes!(""),
                ..Default::default()
            }),
        ...
        let _unifi_address = match results.first() {
            Some(ExecutionResult::Success {
                output: Output::Create(_, Some(address)),
                ..
            }) => *address,
            _ => eyre::bail!("Unifi contract address not found"),
        };
    ...
    async fn test_execute_block() -> eyre::Result<()> {
        ...
        let mut evm = Evm::builder()
            ...
            .with_tx_env(TxEnv {
                ...
                data: WETH::balanceOfCall::new((sender_address,))
                    .abi_encode()
                    .into(),
                ..Default::default()
            })
    ...
    async fn execute_transaction(
        ...
        let block_header = BlockHeader {
            unifiChainId: U256::from(CHAIN_ID),
            sequenceNum: U256::from(sequence),
            gasLimit: U256::from(30_000_000),
            unifiBlockHash: b256!(
                "f172873c63909462ac4de545471fd3ad3e9eeadeec4608b92d16ce6b500704cc"
            ),
        };
